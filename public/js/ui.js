...
export const showCallingDialog = (rejectCallHandler) => {
    const callingDialaog = elements.getCallingDialog(rejectCallHandler);

    // remove all dialog inside HTML dialog element
    const dialog = document.getElementById('dialog');
    dialog.querySelectorAll('*').forEach((dialog) => dialog.remove());
    dialog.appendChild(callingDialaog);
};

export const showInfoDialog = (preOfferAnsweer) => {
    let infoDialog = null;
...
export const removeAllDialogs = () => {
    ...
    dailog.querySelectorAll('*').forEach((dailog) => dailog.remove());
};
